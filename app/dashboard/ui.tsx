  ...
  <div className="min-h-screen bg-slate-50">
   ...
   <div className="mx-auto grid max-w-7xl gap-6 p-4 lg:grid-cols-[260px_1fr]">
    ...
    <main className="space-y-5">
     ...
     {active === 'roles' && <RolePanel users={users} />}
...
function RolePanel({ users }: any) {
 return <section className="rounded-lg bg-white p-5 shadow"><h3 className="font-bold">Assign pending users</h3><div className="mt-4 grid gap-3 md:grid-cols-3">{users.filter((u: any) => u.role === 'PENDING').map((u: any) => <div key={u.id} className="rounded-lg border p-4"><b>{u.name}</b><p className="text-sm">{u.email}</p><div className="mt-3 flex flex-wrap gap-2">{['PARENT', 'TEACHER', 'ADMIN'].map(role => <button key={role} onClick={() => postJson('/api/roles/approve', { userId: u.id, role }).then(ok => ok && location.reload())} className="rounded-lg bg-slate-900 px-3 py-2 text-sm text-white">{role}</button>)}</div></div>)}</div></section>
}
...
function PaymentsPanel({ payments, students, role, submitJson }: any) {
 return <section className="space-y-5">{role !== 'PARENT' && <><form onSubmit={(event) => submitJson(event, '/api/payments')} className="grid gap-3 rounded-lg bg-white p-5 shadow md:grid-cols-2"><h3 className="font-bold md:col-span-2">Create payment</h3><select name="studentId" className="rounded-lg border p-3">{students.map((s: any) => <option key={s.id} value={s.id}>{s.firstName} {s.lastName}</option>)}</select><input name="amount" placeholder="Amount in cents" className="rounded-lg border p-3" /><input name="dueDate" type="date" className="rounded-lg border p-3" /><select name="status" className="rounded-lg border p-3"><option>DUE</option><option>PAID</option><option>OVERDUE</option></select><button className="rounded-lg bg-violet-600 p-3 font-semibold text-white md:col-span-2">Create payment</button></form><button onClick={() => postJson('/api/payments/remind', {}).then(ok => ok && location.reload())} className="rounded-lg bg-amber-600 px-4 py-2 font-semibold text-white">Remind parents with due payments</button></>}{payments.map((p: any) => <div key={p.id} className="rounded-lg bg-white p-5 shadow"><b>{p.student.firstName} {p.student.lastName}</b><p>Amount: ${(p.amount / 100).toFixed(2)} - Status: {p.status}</p><p className="text-sm">Evidence: {p.receiptUrl || 'None'}</p>{role === 'PARENT' && <form onSubmit={(event) => submitJson(event, '/api/payments')} className="mt-3 grid gap-2"><input type="hidden" name="paymentId" value={p.id} /><input name="receiptUrl" placeholder="Payment evidence URL" className="rounded-lg border p-3" /><textarea name="evidenceNote" placeholder="Payment note" className="rounded-lg border p-3" /><button className="rounded-lg bg-violet-600 p-3 text-white">Send payment evidence</button></form>}</div>)}</section>
}
