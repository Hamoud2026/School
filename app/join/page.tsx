...
export default function PublicJoin() {
 ...
 async function submit(event: FormEvent<HTMLFormElement>) {
  ...
  const response = await fetch('/api/join/public', {
   method: 'POST',
 ...
 return (
  <main className="flex min-h-screen items-center justify-center bg-slate-50 p-6">
   <section className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow">
    <h1 className="text-3xl font-bold text-slate-900">Public Student Registration</h1>
    ...
    <form onSubmit={submit} className="mt-6 grid gap-3 md:grid-cols-2">
     ...
     <input name="studentLevel" placeholder="Student level" className="rounded-xl border p-3" />
     <textarea name="note" placeholder="Notes or requests" className="rounded-xl border p-3 md:col-span-2" />
     <button disabled={loading} className="rounded-xl bg-violet-600 p-3 font-semibold text-white disabled:bg-violet-300 md:col-span-2">
      {loading ? 'Sending...' : 'Register student'}
     </button>
